        ...
        for sigungu, (lat, lon) in sigungu_dict.items():
            for i in range(np.random.randint(5, 12)):
                ...
                records.append({
                    ...
                    "lat": lat + np.random.uniform(-0.03, 0.03), 
                    "lon": lon + np.random.uniform(-0.03, 0.03),
...
def get_precise_geometry(sido_name, sigungu_name):
    try:
        if sigungu_name != "전체" and sido_name != "전국":
            ...
            gdf = gpd.read_file(url)
            if sido_name == "전국":
                target_geom = gdf.geometry.unary_union 
            else:
                target_geom = gdf[gdf['name'] == sido_name].geometry.unary_union

        # 전 세계를 덮는 거대한 박스에서 선택한 지역만 뻥 뚫어냄 (Spotlight 효과)
...
with st.sidebar:
    ...
    # 🚨 [핵심 업데이트] 텍스트 입력으로 위치 검색 시뮬레이션
    custom_address = st.text_input("상세 위치 입력", placeholder="예: 서교동, 홍대입구역")
...
# 커스텀 주소 검색이 있을 경우 (반경 필터 시뮬레이션)
if custom_address and not fac_filtered.empty:
    # MVP 시뮬레이션: 검색어가 입력되면 해당 구의 중심점 기준으로 데이터를 더 좁게 자름
    center_lat, center_lon = fac_filtered['lat'].mean(), fac_filtered['lon'].mean()
    # 반경 좁히기 (위경도 상 대략 1km = 0.009)
    radius_deg = search_radius * 0.009
    fac_filtered = fac_filtered[
        (fac_filtered['lat'] >= center_lat - radius_deg) & (fac_filtered['lat'] <= center_lat + radius_deg) &
        (fac_filtered['lon'] >= center_lon - radius_deg) & (fac_filtered['lon'] <= center_lon + radius_deg)
    ]
...
tab1, tab2, tab3 = st.tabs(["🗺️ 청소년 안전망 집중 지도", "📊 지역 통계 및 실태 비교", "🆘 긴급 도움받기"])

# ──────────────────────────────────────────────
# 탭 1: 안전망 집중 지도 (Spotlight 기능)
# ──────────────────────────────────────────────
with tab1:
    ...
    # 맵뷰 포커싱 (검색 반경에 따라 줌 레벨 다이내믹 변경)
    if fac_filtered.empty:
        map_center = [37.5665, 126.9780]; zoom_level = 11.0
    else:
        map_center = [fac_filtered['lat'].mean(), fac_filtered['lon'].mean()]
        if custom_address:
            zoom_level = 14.5 # 상세 주소 입력 시 아주 깊게 줌인
        elif selected_sigungu != "전체" and selected_sido != "전국":
            zoom_level = 13.0 # 구 단위 줌인 (기존보다 더 확대)
    ...
    # 🚨 [핵심기능] 타 지역 완전 차단 (Spotlight) 레이어
    if mask_json is not None:
        layers.append(pdk.Layer(
            "GeoJsonLayer", 
            mask_json, 
            get_fill_color=theme['mask_color'], 
            stroked=True,
            get_line_color=theme['accent_color'].replace('#', '').replace(theme['accent_color'], '[56, 189, 248, 255]'), # 경계선 하이라이트
    ...
    for threat in selected_threats:
        ...
        layers.append(pdk.Layer(
            ...
            get_fill_color="color", pickable=True, opacity=0.7 
        ))
    ...
    # 세련된 툴팁 정보
    tooltip_html = f"""
    <div style='background-color:{theme['card_bg']}; padding:12px; border-radius:8px; border: 1px solid {theme['card_border']}; color:{theme['text_color']}; font-family:sans-serif; max-width: 250px; box-shadow: 0 4px 6px rgba(0,0,0,0.3);'>
