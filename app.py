...
# 🚨 [핵심 1] 대한민국 영토 마스크(가림막) 생성 함수
@st.cache_resource
def get_korea_geometry():
    # 통계청 기반 대한민국 행정구역 경계선 다운로드
    ...
    try:
        ...
        kr_geom = kr_gdf.geometry.unary_union # 대한민국을 하나의 거대한 영토로 합침
        
        # 전 세계를 덮는 거대한 상자에서 대한민국 모양만 구멍 뚫기
        world_box = box(-180, -90, 180, 90)
        mask_geom = world_box.difference(kr_geom)
        mask_gdf = gpd.GeoDataFrame(geometry=[mask_geom], crs="EPSG:4326")
        
        # 스트림릿 충돌 방지를 위해 JSON 형태로 반환
...
with tab1:
    ...
    st.caption("💡 마우스 **우클릭 후 드래그**하면 지도를 3D로 기울여 산맥 지형과 입체적인 데이터를 확인할 수 있습니다.")
    ...
    # 🚨 [핵심 2] 바다에 빠진 점 지우기 (쿠키 커터 적용)
    if kr_geom is not None:
        # 무작위 데이터를 공간 데이터로 변환 후, 대한민국 영토(kr_geom) 안쪽에 있는 점만 남김
    ...
    # 🚨 [핵심 3] 주변 국가와 바다를 덮어버리는 암막 레이어 (가장 밑바닥에 배치)
    if mask_json is not None:
        layers.append(pdk.Layer(
            "GeoJsonLayer",
            mask_json,
            get_fill_color=[14, 17, 23, 255], # 스트림릿 기본 다크 테마 배경색과 동일하게 칠하여 완벽 은폐
            stroked=False,
            pickable=False
        ))
    ...
    st.pydeck_chart(pdk.Deck(
        ...
        effects=[pdk.TerrainEffect()] # ⛰️ Python PyDeck 0.8 버전 이상에서 지원하는 실제 산맥 지형 굴곡 구현!
    ))
